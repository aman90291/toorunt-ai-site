...
export const FAQ: readonly FaqEntry[] = [
  ...
  },
  {
    ...
    a: "Five phases. Ingest & scope — it watches your board, picks up the ticket and resolves the right repo. Plan & approval gate — it posts an implementation plan to Jira and stops. Implement & prove — it writes the change and the tests until green. Review orchestration — a second agent and a peer bot review it. Merge & watch — it merges on verified-green and then watches CI and production.",
    category: "How it works",
  ...
  },
  {
    ...
    a: "No. It posts an implementation plan to Jira — files, approach, risks, acceptance criteria — and stops at the first human gate. Gate 04 does not open until a human has seen the plan.",
    category: "How it works",
  ...
  },
  {
    ...
    a: "Yes, and it has to. It implements the change and writes tests until green. For a bug it writes a reproduction test that is RED before the fix and GREEN after — proof the fix actually fixes the bug. Gate 09 checks that every changed file has covering tests; missing ones are written before the PR opens.",
    category: "How it works",
  ...
  },
  {
    ...
    a: "Tickets are claimed atomically — two bots can race, one wins, the other moves on. Every bot declares the files it will touch before it starts, and overlapping claims queue instead of colliding. A bot blocked on a dependency parks the ticket with its state intact so any peer can resume it. Escalation follows CODEOWNERS, git blame, Jira roles and on-call — a bounded ladder that always terminates.",
    category: "How it works",
  ...
  },
  {
    id: "how-fast",
    q: "How fast is it?",
    a: `${RECEIPT[0].value} from idea to deployed product in the July 2026 founding run — one live evening. Against a manual SDLC's one-to-two weeks, that is a 10–20× cycle-time compression: weeks become hours.`,
    category: "How it works",
  ...
  },
  {
    ...
    a: "Two gates handle it. Novelty calibration means routine work ships while unfamiliar territory forces extra deliberation and a human merge. Risk & confidence scores every change, and anything past the cap escalates to a human. Money-moving, irreversible or cross-team actions hit a hard escalation contract — a human signs, or it doesn't happen.",
    category: "Governance",
  ...
  /* ── Pricing ────────────────────────────────────────────────────── */
  {
    ...
    a: `$${ECON.price} per gate-passed, complexity-normalized merged pull request — so a one-line fix and an architectural change don't price the same. Per-seat pricing is available for high-volume teams, and enterprise brings your own vault, SSO and isolated deployment.`,
    category: "Pricing",
  ...
  {
    id: "rejected-pr-cost",
    q: "What if the PR is rejected — do I still pay?",
    a: "No. You're only billed for work that shipped and passed review. A rejected pull request costs nothing.",
    category: "Pricing",
    tags: ["rejected", "failed", "refund", "pay", "billing", "free", "nothing"],
    href: "/pricing/",
  },
  {
    id: "runaway-spend",
    q: "Can spend run away?",
    a: "No. Every ticket carries a hard cost cap enforced in the product, and the whole fleet has a kill switch. Compute is metered on an append-only ledger you can see.",
    category: "Pricing",
    tags: ["spend", "budget", "cap", "runaway", "limit", "overspend", "control", "ledger"],
    href: "/pricing/",
  },
  {
    id: "cogs",
    q: "What's your actual compute cost?",
    a: `$${ECON.cogsLow}–$${ECON.cogsHigh} per merged PR. Only three pipeline layers call a model; the rest is deterministic code. We show you the ledger — the margin isn't a mystery.`,
    category: "Pricing",
    tags: ["compute", "cogs", "margin", "tokens", "llm cost", "ledger", "unit economics"],
    href: "/pricing/",
  },
  {
    id: "vs-human",
    q: "How does that compare to a human engineer?",
    a: `Loaded engineer cost runs $${ECON.humanLow}–$${ECON.humanHigh} per pull request over a one-to-two week cycle. At $${ECON.cogsLow}–$${ECON.cogsHigh} of compute per merged PR that is roughly 90% lower cost per unit of shipped, reviewed work — and a 10–20× cycle-time compression.`,
    category: "Pricing",
    tags: ["compare", "human", "engineer", "savings", "roi", "cheaper", "versus", "salary"],
    href: "/pricing/",
  },
  {
    id: "merged-pr-definition",
    q: "What counts as a merged PR?",
    a: "A pull request that cleared the full gate chain and merged under your branch protection. Complexity-normalized, so a one-line fix and an architectural change don't price the same.",
    category: "Pricing",
    tags: ["merged pr", "unit", "billing", "counts", "definition", "what is"],
    href: "/pricing/",
  },
